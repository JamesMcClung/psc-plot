...
- `MetadataAdaptor` — wraps `apply` to also append name fragments and modify the active variable's `Dimension` in `var_info` (used to derive saved filenames and axis labels). Override `get_modified_display_latex(metadata)` and/or `get_modified_unit_latex(metadata)`; both receive the current `metadata` so they can inspect e.g. `active_key` and `active_var_info`.
...
- `PREFIXED_REGISTRY` — keyed by `(prefix, active_key)` for field and particle variables (e.g. `("pfd", "hx_fc")` → `Dimension(name="B_x", ...)`).
