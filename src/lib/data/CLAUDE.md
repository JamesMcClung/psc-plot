...
**It stays lazy** — materializing is the user's call, via `--compute` *after* `--bin`. Deciding it here was once necessary: under the single `histogramdd` every output element depended on every partition, so slicing `t=frame` culled nothing and an animation cost `n_frames + 2` full passes over the particle files (203 for a 201-frame run). The per-step stacking above fixed that — each `t` bin is its own slice of the stack, so a frame culls to that step's partitions, and the remaining lazy cost is one extra full pass (the `bounds()` reduction) plus a `compute()` per frame. What eager binning still buys is that one pass and 200 graph optimizations; what it costs is the whole `prod(nbins) × n_t` grid resident from `pull()` onward (211 MB for `--bin y=512 uy=256` over 201 steps), and it defeats any later `--idx`/`--reduce` that would have culled the stack. `tests/test_bin.py` pins both halves: per-frame culling, and one-pass materialization under `-c`.
