...
class Scatter(MetadataAdaptor):
    ...
    def apply_field(self, data: Field) -> LazyList:
        ...
        vars = {data.active_key: da.ravel(data.require_active_subdata())} | dict(zip(data.dims, (da.ravel(coord_grid) for coord_grid in coord_grids)))
