...
def _guess_bin_edgess(data: List, varname_to_nbins: dict[str, int | None]) -> list:
    ...
    for varname, nbins in varname_to_nbins.items():
        ...
        elif varname in DIM_DEFAULTS and (DIM_DEFAULTS[varname].geometry == "polar:theta" or DIM_DEFAULTS[varname].geometry == "spherical:phi"):
            varname_to_edges[varname] = np.linspace(-np.pi, np.pi, nbins + 1, endpoint=True)
        elif varname in DIM_DEFAULTS and DIM_DEFAULTS[varname].geometry == "spherical:theta":
